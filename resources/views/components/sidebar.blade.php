    ...
    <aside id="logo-sidebar" class="fixed top-0 left-16 z-40 w-64 h-screen border-r transition-transform -translate-x-full bg-white dark:bg-gray-800">
        <div class="h-full px-3 py-4 overflow-y-auto">
            ...
            <div>
                ...
                <form action="{{ route('logout') }}" method="POST" class="flex items-center p-2 pl-4 rounded-lg text-red-600 hover:bg-red-100 dark:hover:bg-red-700">
                    @csrf
                    <button type="submit">
...
<nav id="bottom-navbar" class="fixed bottom-0 left-0 z-50 w-full bg-white dark:bg-gray-800 border-t shadow-md sm:hidden">
    <ul class="flex justify-around py-2 text-gray-600">
        <li>
            <a href="{{ route('dashboard') }}" class="flex flex-col items-center {{ request()->routeIs('dashboard') ? 'text-blue-500 dark:text-blue-500' : '' }} dark:text-white">
                <i class="fa-solid fa-house text-xl"></i>
                <span class="text-xs">Home</span>
            </a>
        </li>
        <li>
            <a href="{{ route('tasks.index') }}" class="flex flex-col items-center {{ request()->routeIs('tasks.index') ? 'text-blue-500 dark:text-blue-500' : '' }} dark:text-white">
                <i class="fa-solid fa-list-check text-xl"></i>
                <span class="text-xs">Tasks</span>
            </a>
        </li>
        <li>
            <a href="{{ route('history') }}" class="flex flex-col items-center {{ request()->routeIs('history') ? 'text-blue-500 dark:text-blue-500' : '' }} dark:text-white">
                <i class="fa-regular fa-note-sticky text-xl"></i>
...
<script>
    document.getElementById('toggle-sidebar').addEventListener('click', function () {
        ...
        if (sidebar.classList.contains('-translate-x-full')) {
            ...
            navbar.classList.remove('sm:left-16');
            ...
            navbar.classList.add('sm:left-16');
        }
    });
</script>